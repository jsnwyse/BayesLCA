blca.boot <- function( X, G, formula=NULL, ncat=NULL, alpha=1, beta=1, delta=1, 
                       ...
                       iter=2000,  B=100, verbose=FALSE, verbose.update=100, conv=1e-06, small=1e-10, MAP=TRUE)
{
  ...
  for( j in 1:M )
  {
    if( model.indicator[j] )
    {
      ...
      for( g in 1:G )
      {
        bm <- var.probs.l[[l]][ tt+g , , drop=FALSE]
  ...
  for( b in 1:B )
  {
    ...
    for( j in 1:M  )
    {
      ...
      if( model.indicator[j] )
      {
        var.probs.l[[l]][ it + 1:G , ] = var.probs.l[[l]][ it + o , , drop=FALSE] # final reorder as label swapping done
        l <- l+1
      }
    }
  }
