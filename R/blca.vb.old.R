...
{	
	...
	while(abs(eps)>conv || counter< 20)
	{
		...
		l4<- sum((alpha-1)*Elog.zeta1 +(beta-1)*Elog.zeta2)+sum(lgamma(alpha+beta))-sum(lgamma(alpha),lgamma(beta))

		l5<- sum((zeta1-1)*Elog.zeta1 + (zeta2-1)*Elog.zeta2)+sum(lgamma(zeta1+zeta2))-sum(lgamma(zeta1),lgamma(zeta2))
		
		l6<- sum((gamma-1)*Elog.gamma)+lgamma(sum(gamma))-sum(lgamma(gamma))
		
		l7<- sum(counts.n * xlogy(Z,Z))
